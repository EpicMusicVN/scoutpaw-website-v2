...
import { breadcrumbSchema, videoObjectSchema } from "@/lib/seo/structured-data";
...
export default async function WatchPage() {
  ...
  // VideoObject per surfaced video (those without an uploadDate are skipped),
  // plus a breadcrumb trail for the page.
  const siteUrl = getSiteUrl();
  const videoSchemas = videos
    .map((v) => videoObjectSchema(v, siteUrl))
    .filter((node): node is NonNullable<typeof node> => node !== null);
  const watchBreadcrumb = breadcrumbSchema([
  ...
  return (
    <>
      <JsonLd data={[...videoSchemas, watchBreadcrumb]} />
